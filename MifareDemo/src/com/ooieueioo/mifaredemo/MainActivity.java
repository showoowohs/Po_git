package com.ooieueioo.mifaredemo;
...
import com.ooieueioo.mifaredemo.dataobject.mifare.MifareBlock;
import com.ooieueioo.mifaredemo.dataobject.mifare.MifareClassCard;
import com.ooieueioo.mifaredemo.dataobject.mifare.MifareSector;
import com.ooieueioo.mifaredemo.dataobject.mifare.util.Converter;

public class MainActivity extends ListActivity {

	public static final byte[] Po_KEY_DEFAULT = { (byte) 0x5E, (byte) 0xC7,
			(byte) 0x93, (byte) 0x8F, (byte) 0x14, (byte) 0x0A };
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		Po_TV1 = (TextView) findViewById(R.id.Po_TV1);
		...
		PO_resolveIntent(intent);

	}

	// Po add
	void PO_resolveIntent(Intent intent) {
...

		{
...

			{
				...
				Toast.makeText(this, "檢測到卡片，讀取中。。。", Toast.LENGTH_SHORT).show();
...

				{
					...
					// Log.e("Po add", "secCount =" + secCount);

					auth = mfc.authenticateSectorWithKeyA(15,
					// MifareClassic.KEY_DEFAULT);// 驗證KeyA
							Po_KEY_DEFAULT);// 驗證KeyA

					if (auth)

					{

						// 讀取卡片的第60格 , 15 sector block 0
						byte[] data = mfc.readBlock(60);

						String Po_show_data = Converter.getHexString(data,
								data.length);

						Log.i("Po add", "O");
						Log.i("Po add", "data is :" + Po_show_data);
						this.Po_TV1.setTextSize(24);
						this.Po_TV1.setText(Po_show_data);
					} else {
						Log.e("Po add", "X");
					}
		...
		if (NfcAdapter.ACTION_TECH_DISCOVERED.equals(action)) {
			...
			try { // 5.1) Connect to card
				...
				for (int j = 0; j < secCount; j++) {
					...
					setTitle(MifareClassic.KEY_DEFAULT.toString());
